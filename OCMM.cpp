...
int writeDepthHeatMapPng(const std::string file_path, const cv::Mat_<float> depth) {
	// Step 1: Filter out invalid depths (if needed)
	cv::Mat valid_depth = depth.clone();

	valid_depth.convertTo(valid_depth, CV_8UC1); // Fallback if all depths are equal

	// Step 4: Apply a perceptually uniform color map (e.g., VIRIDIS or TURBO)
	cv::Mat heat_map;
	cv::applyColorMap(valid_depth, heat_map, cv::COLORMAP_JET); // Better than JET for depth

	// Step 5: Save the heat map
	if (!cv::imwrite(file_path, heat_map)) {
...
{
	// Step 1: Normalize the costMap values to the range [0, 255] for visualization
	cv::Mat cost_normalized;
	cv::normalize(costMap, cost_normalized, 0, 255, cv::NORM_MINMAX);

	// Step 2: Convert the normalized cost map to an 8-bit format
	cv::Mat cost_8bit;
	cost_normalized.convertTo(cost_8bit, CV_8UC1);

	// Step 3: Optionally apply a color map, or save as a grayscale image
	cv::Mat output_image;
	cv::applyColorMap(cost_8bit, output_image, cv::COLORMAP_JET); // For heatmap effect

	// Alternative: Uncomment the line below to save as a grayscale image without color mapping
	// output_image = cost_8bit;

	// Step 4: Save the resulting image as a PNG file
	if (!cv::imwrite(file_path, output_image)) {
		std::cout << "Error writing file " << file_path << std::endl;
		return -1;
	}
...
{
	// Step 1: Normalize each channel in the 3D normal matrix to the range [0, 255] for RGB display
	cv::Mat channels[3];
	cv::split(normal, channels);  // Split the normal matrix into x, y, z channels

	// Normalize each channel individually
	cv::normalize(channels[0], channels[0], 0, 255, cv::NORM_MINMAX); // Red channel
	cv::normalize(channels[1], channels[1], 0, 255, cv::NORM_MINMAX); // Green channel
	cv::normalize(channels[2], channels[2], 0, 255, cv::NORM_MINMAX); // Blue channel

	// Convert each normalized channel to 8-bit for display
	channels[0].convertTo(channels[0], CV_8UC1); // Red channel
	channels[1].convertTo(channels[1], CV_8UC1); // Green channel
	channels[2].convertTo(channels[2], CV_8UC1); // Blue channel

	// Merge the channels back into an RGB image
	cv::Mat rgb_image;
	cv::merge(channels, 3, rgb_image);

	// Step 2: Save the RGB image as a PNG
	if (!cv::imwrite(file_path, rgb_image)) {
		std::cout << "Error writing file " << file_path << std::endl;
		return -1;
	}
...
void OCMM::SetGeomConsistencyParams(bool multi_gemetry=false)
{
...
void OCMM::SetHierarchyParams()
{
    params.hierarchy = true;
}

void OCMM::InputInitialization(const std::string &dense_folder, const std::vector<Problem> &problems, const int idx)
{
    images.clear();
    cameras.clear();
	...
	std::cout << "Images collected ... " << std::endl;
	...
	std::cout << "Cameras collected ... " << std::endl;
	...
	//std::cout << "images and cameras collected ... " << std::endl;
	...
	{
		cameras[i].height = images[i].rows;
		cameras[i].width = images[i].cols;

        if (i > 0) 
		{
            max_image_size = problems[problem.src_image_ids[i - 1]].cur_image_size;
        }
		//std::cout << "i-th image available ... " << std::endl;
        if (images[i].cols <= max_image_size && images[i].rows <= max_image_size) 
		{
            continue;
        }
		//std::cout << "images cols and rows are good ... " << std::endl;
	...
	params.depth_min = fmax(0.0f, fmin(64.0f, cameras[0].depth_min * 0.6f));
	params.depth_max = fmin(128000.f, fmax(16000.0f, cameras[0].depth_max * 1.2f));
    std::cout << "depth range [min|max]: " << params.depth_min << " " << params.depth_max << std::endl;
    ...
    if (params.geom_consistency) {
        ...
        result_path << dense_folder << "/OPM-MVS" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
		...
		std::string depth_heatmap_path = result_folder + "/depths.png";
        ...
        for (size_t i = 0; i < num_src_images; ++i) {
            std::stringstream result_path;
            result_path << dense_folder << "/OPM-MVS" << "/2333_" << std::setw(8) << std::setfill('0') << problem.src_image_ids[i];
            std::string result_folder = result_path.str();
            std::string depth_path = result_folder + suffix;
            cv::Mat_<float> depth;
...
void OCMM::CudaSpaceInitialization(const std::string &dense_folder, const Problem &problem)
{
    ...
    if (params.geom_consistency) {
		...
		std::string depth_heatmap_path = result_folder + "/depths.png";
		std::string normal_heatmap_path = result_folder + "/normals.png";
		std::string cost_heatmap_path = result_folder + "/costs.png";
        cv::Mat_<float> ref_depth;
    ...
    if (params.hierarchy) {
		...
		cudaMemcpy(costs_cuda, costs_host, sizeof(float4) * height * width, cudaMemcpyHostToDevice);
    }
}

int OCMM::GetReferenceImageWidth()
{
    return cameras[0].width;
}

int OCMM::GetReferenceImageHeight()
{
    return cameras[0].height;
}

cv::Mat OCMM::GetReferenceImage()
{
    return images[0];
}

float4 OCMM::GetPlaneHypothesis(const int index)
{
    return plane_hypotheses_host[index];
}

float OCMM::GetCost(const int index)
{
 ...
 void JBUAddImageToTextureFloatGray ( std::vector<cv::Mat_<float>>  &imgs, cudaTextureObject_t texs[], cudaArray *cuArray[], const int &numSelViews)
{
 ...
 void JBU::InitializeParameters(int n, const cv::Mat_<float4> &src_normal)
 {
     InitializeParameters(n);

	 normal_h = (float4*)malloc(sizeof(float4) * n);
	 cudaMalloc((void**)&normal_d, sizeof(float4) * n);

	 const int src_n = jp_h.s_height * jp_h.s_width;
	 cudaMalloc((void**)&src_normal_d, sizeof(float4) * src_n);
	 cudaMemcpy(src_normal_d, src_normal.ptr<float4>(), sizeof(float4) * src_n, cudaMemcpyHostToDevice);
	 cudaDeviceSynchronize();
 }
...
void RunJBU(const cv::Mat_<float>  &scaled_image_float, const cv::Mat_<float> &src_depthmap, const std::string &dense_folder , const Problem &problem)
{
	...
	const float dcol = scaled_image_float.cols / src_depthmap.cols;
	const float drow = scaled_image_float.rows / src_depthmap.rows;
	const float icol = (src_depthmap.cols / scaled_image_float.cols);
	const float irow = (src_depthmap.rows / scaled_image_float.rows);
	...
	for (int i = 2; i < JBU_NUM; ++i) {
		imgs[i] = cv::Mat_<float>::zeros(1, 1);
	}
	...
	jbu.jp_h.imgupscale = sqrt(icol * irow); //fmaxf(irow, icol);
	jbu.jp_h.imgdownscale = sqrt(dcol * drow); //fminf(drow, dcol);
    JBUAddImageToTextureFloatGray(imgs, jbu.jt_h.imgs, jbu.cuArray, JBU_NUM);

    jbu.InitializeParameters(rows * cols);
	int scale = problem.num_downscale;
    jbu.CudaRun(scale);
