#!/usr/bin/env python
"""
Copyright 2025, Ray Khuboni. Camera model conversion which is provided by COLMAP and Qingshan Xu.
...
def processing_single_scene(args):
    ...
    for i in range(num_images):
        ...
        for p3d_id in images[i+1].point3D_ids:
            ...
            zs.append(np.isscalar(transformed[2]))
        zs_sorted = sorted(zs)
        # relaxed depth range
    ...
    #num_view = min(10, len(images) - 1)
    #num_view = len(images) - 1
    for i in range(len(images)):
