...
void ProcessProblem(const std::string &dense_folder, const std::vector<Problem> &problems, const int idx, bool geom_consistency, bool hierarchy, const int scale, bool multi_geometrty=false)
{
    ...
    mkdir(result_folder.c_str());//mkdir(result_folder.c_str(), 0777);
	std::cout << "Folder created ... " << std::endl;
    OCMM OCMM;
    if (geom_consistency) {
        OCMM.SetGeomConsistencyParams(multi_geometrty);
    }
    if (hierarchy) {
        OCMM.SetHierarchyParams();
    }
	std::cout << "Geometric consistency checked... " << std::endl;
	std::cout << "Dense Dir: " << dense_folder <<" Problems [idx]:  "<<idx << std::endl;
	OCMM.InputInitialization(dense_folder, problems, idx);
	std::cout << "Initialization done ... " << std::endl;
	OCMM.CudaSpaceInitialization(dense_folder, problem);
	std::cout << "Cudaspace created ... " << std::endl;
	OCMM.RunPatchMatch();
	std::cout << "PatchMatch done ... " << std::endl;

    const int width = OCMM.GetReferenceImageWidth();
    const int height = OCMM.GetReferenceImageHeight();

    cv::Mat_<float> depths = cv::Mat::zeros(height, width, CV_32FC1);
	cv::Mat_<float> disparity_depths = cv::Mat::zeros(height, width, CV_32FC1);
    ...
    for (int col = 0; col < width; ++col) {
        for (int row = 0; row < height; ++row) {
            ...
            float4 plane_hypothesis = OCMM.GetPlaneHypothesis(center);
			float point3D[3];
			depths(row, col) = plane_hypothesis.w;
            normals(row, col) = cv::Vec3f(plane_hypothesis.x, plane_hypothesis.y, plane_hypothesis.z);
            costs(row, col) = OCMM.GetCost(center);
        }
    }

    std::string suffix = "/depths.dmb";
	std::string img_suffix = "/depths.png";
    if (geom_consistency) {
        suffix = "/depths_geom.dmb";
		img_suffix = "/depths_geom.png";
    }
    std::string depth_path = result_folder + suffix;
    std::string normal_path = result_folder + "/normals.dmb";
    std::string cost_path = result_folder + "/costs.dmb";
	std::string depth_heatmap_path = result_folder + img_suffix;
	...
	//images saved for viewing purposes
	writeDepthHeatMapPng(depth_heatmap_path, depths);
	writeDepthHeatMapPng(depth_heatmap_path, disparity_depths);
	writeNormalHeatMapPng(normal_heatmap_path, normals);
	writeCostMapPng(cost_heatmap_path, costs);
    std::cout << "Processing image " << std::setw(8) << std::setfill('0') << problem.ref_image_id << " done!" << std::endl;
}

void JointBilateralUpsampling(const std::string &dense_folder, const Problem &problem, int ocmm_size)
{
    std::stringstream result_path;
    result_path << dense_folder << "/OPM-MVS" << "/2333_" << std::setw(8) << std::setfill('0') << problem.ref_image_id;
    std::string result_folder = result_path.str();
    ...
    std::string depth_geom_path = result_folder + "/depths_geom.dmb";
	std::string depth_heatmap_path = result_folder + "/depths.png";
	std::string normal_heatmap_path = result_folder + "/normals.png";
	std::string cost_heatmap_path = result_folder + "/costs.png";
	std::string depth_heatmap_geo_path = result_folder + "/depths_geom.png";
    cv::Mat_<float> ref_depth;
	cv::Mat_<float> depths;
	cv::Mat_<cv::Vec3f> normals;
	cv::Mat_<float> costs;
    readDepthDmb(depth_geom_path, ref_depth);
    ...
    const float factor_x = static_cast<float>(ocmm_size) / image_float.cols;
    const float factor_y = static_cast<float>(ocmm_size) / image_float.rows;
	const float factor = std::min(factor_x, factor_y);
	std::cout << "image [sx , sy, sxy]: " << factor_x << "," << factor_y << "," << sqrt(factor_x * factor_y) << std::endl;
	...
	std::cout << "image [rows , cols]: " << image_float.rows << "," << image_float.cols <<" scaled image [r,c]: "<< new_rows << "," << new_cols << "." << std::endl;
    std::cout << "Run JBU for image " << problem.ref_image_id <<  ".jpg" << std::endl;
    RunJBU(scaled_image_float, ref_depth, dense_folder, problem);
	std::cout << "Joint Bilateral Upsample for image " << problem.ref_image_id << ".jpg complete ..." << std::endl;
	writeDepthHeatMapPng(depth_heatmap_geo_path, ref_depth);
	writeDepthHeatMapPng(depth_heatmap_path, depths);
	writeNormalHeatMapPng(normal_heatmap_path, normals);
	writeCostMapPng(cost_heatmap_path, costs);
}
...
void RunFusion(std::string &dense_folder, const std::vector<Problem> &problems, bool geom_consistency)
{
	...
	for (size_t i = 0; i < num_images; ++i) {
		...
		std::stringstream result_path;
		result_path << dense_folder << "/OPM-MVS" << "/2333_" << std::setw(8) << std::setfill('0') << problems[i].ref_image_id;
		std::string result_folder = result_path.str();
		std::string suffix = "/depths.dmb";
		if (geom_consistency) {
			suffix = "/depths_geom.dmb";
	...
	std::string ply_path = dense_folder + "/OPM-MVS/OPM-MVS_model.ply";
...
{
    if (argc < 2) {
        std::cout << "USAGE: OPM-MVS dense_folder" << std::endl;
        return -1;
    }
    ...
    std::string output_folder = dense_folder + std::string("/OPM-MVS");
    ...
    int max_num_downscale = ComputeMultiScaleSettings(dense_folder, problems);
     ...
     while (max_num_downscale >= 0) {
        ...
        if (flag == 0) {
			...
			std::cout << "Process Problem - Stage 1" << std::endl;
            for (size_t i = 0; i < num_images; ++i) {
                ProcessProblem(dense_folder, problems, i, geom_consistency ,hierarchy, max_num_downscale);
            }
            geom_consistency = true;
			std::cout << "Process Problem - Stage 2: Multi-Geometry" << std::endl;
            for (int geom_iter = 0; geom_iter < geom_iterations; ++geom_iter) {
                ...
                for (size_t i = 0; i < num_images; ++i) {
                    ProcessProblem(dense_folder,  problems, i, geom_consistency, hierarchy, max_num_downscale, multi_geometry);
                }
            }
        }
        else {
			std::cout << "Process Problem - Stage 3: Joint Bilateral Upsampling" << std::endl;
            for (size_t i = 0; i < num_images; ++i) {
            ...
            hierarchy = true;
            geom_consistency = false;
			std::cout << "Process Problem - Stage 3-1" << std::endl;
            for (size_t i = 0; i < num_images; ++i) {
                ProcessProblem(dense_folder,  problems, i, geom_consistency, hierarchy, max_num_downscale);
            }
            hierarchy = false;
            geom_consistency = true;
			std::cout << "Process Problem - Stage 3-2: Multi-Geometry" << std::endl;
            for (int geom_iter = 0; geom_iter < geom_iterations; ++geom_iter) {
                if (geom_iter == 0) {
                    multi_geometry = false;
                }
	...
	std::cout << "Process Problem - Stage 4: Fusion" << std::endl;
	geom_consistency = true;
